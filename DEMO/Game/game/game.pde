...
String HOST_IP="192.168.7.1"; //ip address
UDP udp;
...
void draw() {
  ...
  if(t==0) {
    ...
    float barsize=75.;
    strokeWeight(3);
    line(x_pos-barsize,heightbounce,x_pos+barsize,heightbounce);
    strokeWeight(1);
    ...
    if (y>=heightbounce && y<heightbounce+10) {
      ...
      if (x>(x_pos-barsize) && x< (x_pos-(1./3)*barsize) && dx>0 && dy>0) {
        dy*=-0.95;
        dx*=-1.05;
      }
      //left part - right left - get more horisontal
      else if (x>(x_pos-barsize) && x< (x_pos-(1./3)*barsize) && dx<0 && dy>0) {
        dy*=-0.95;
        dx*=1.05;
      }
      //right part - left right - get more horisontal
      else if (x>(x_pos+(1./3)*barsize) && x< (x_pos+barsize) && dx>0 && dy>0) {
        dy*=-0.95;
        dx*=1.05;
      }
      ...
      else if (x>(x_pos-(1./3)*barsize) && x< (x_pos+(1./3)*barsize) && dy>0) {
        dy*=-1.05;
        dx*=0.95;
      }
    }
