...
void main(int argc, char* argv[])
{
	...
	fprintf(stdout,"Jrunner compiled for RedCape Version   ");
#ifdef  BEAGLE_BONE
	fprintf(stdout,"A \n");
#else
	fprintf(stdout,"A1 \n ");
...
void ProcessFileInput(int finputid, int idcode)
{
	...
	pBuffer = NULL;
	if ((pBuffer = (unsigned char *)malloc (file_size)) == NULL) {
		fprintf( stderr, "Error: File buffer could not be allocated!" );
		return;
	}

	if (jb_fread(finputid, pBuffer, file_size) != 1) {
	...
	/* Loop through every single byte */
	while (pPos < pBuffer+file_size) {
		one_byte = *pPos++;

		/* Program a byte,from LSb to MSb */
		for (j=0; j<8; j++ ) {

			// output lsb to tdi port
			if (one_byte & 0x01)
				FpgaIspSetTDI();
			else
				FpgaIspClearTDI();
...
{
	// decode signal selection and data state
	// call appropriate io port function

//	printf("DATAOUT: 0x%x\n", gpio5_readRegister(GPIO_DATAOUT));
	...
	// if clock to be toggled call io functions
